        ...
        <>
            ...
            <div className="flex flex-col gap-5">
                {inspirations.map(({ name, github, portfolio }: InspirationProps, index: number) => (
                    <div key={index} className="flex flex-row gap-3 md:gap-10 justify-between">
                        <a className="text-xl md:text-2xl text-white font-bold transition-all duration-300 hover:scale-110 hover:text-red" href={portfolio}>{name}</a>
