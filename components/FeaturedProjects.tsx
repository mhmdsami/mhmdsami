...
const Project = ({ title, children, image, deps, repo }: ProjectProps) => {
    return (
        <div id="projects" className="bg-gray-200 rounded-xl p-5 mt-4 mb-7 mr-7">
            <div className="text-lg font-bold">{title}</div>
            <div className="flex flex-col gap-3">
                {children}
                <a href={deps}><img src={image} alt="scoreboard" className="rounded-md"/></a>
            </div>
...
const FeaturedProjects = () => {
    return (
        <div id="projects" className="mt-[5vh] mb-10">
            <div className="text-lg font-bold uppercase">Featured Projects</div>
